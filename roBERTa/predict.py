...
from tools.HTML_handling import html_tag_dict
...
def setup_model_and_tokenizer(classification: str, modelversion: str):
    ...
    model_name = f"gerbejon/roberta-html-nodes-{classification}-classifier-{modelversion}"
...
def preprocess(node, xpath: str, modelversion: str):
    ...
    xpath_translated = 'From ' + ' to '.join(
        [tag_description(item) for item in xpath.split('/') if len(tag_description(item)) > 0]
    )
    res = re.search(r'(?<=class=")[^"]*(?=")', str(node))
    if res:
        tag_class = ', '.join(res.group().split(' '))
    else:
        tag_class = 'No classes'
    if modelversion == 'v1':
        return 'Tag path: ' + xpath_translated + '. Tag classes are: ' + tag_class
    elif modelversion == 'v2':
        return 'Tag path: ' + '-'.join(xpath.split('/')) + ' Tag classes are: ' + tag_class
    return None
...
def run_prediction(args):
    # Root paths
    rootdir = os.getcwd() if os.getcwd().split("/")[-1] != "roBERTa" else os.path.abspath(os.path.join(os.getcwd(), ".."))
    outdir = os.path.join(rootdir, f"roBERTa/results/{args.modelversion}/result_out")
    ...
    for item in progressbar(annotations):
        ...
        out_page_dir = os.path.join(outdir, f"{item['id']}_roberta_{args.modelversion}")
...
if __name__ == "__main__":
    parser = get_common_parser("Run inference using fine-tuned RoBERTa model")
